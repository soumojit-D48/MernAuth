...
export const AppContextProvider = (props) => { // This is a provider component. You'll wrap your entire app (or part of it) with this so that all child components can access the context.
    ...
    useEffect(() => { // whenever the page loaded it will check the auuth status
        getAuthState() // if true then change the navbar, have to remove the login button cause user is already login
    },[])

    const value = { // grouping all the state values and setters into one object to pass to the provider.
        backendUrl,
    ...
    return ( // This wraps child components with your context provider, making all values available inside any component using useContext(AppContent).
